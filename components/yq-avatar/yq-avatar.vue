		...
		<view class="oper-wrapper" :style="{display: styleDisplay}">
			<view class="oper">
				<view class="btn-wrapper" v-if="showOper">
					...
					<view @click="fUpload"  hover-class="hover" :style="{width: btnWidth}"><text>{{uptext}}</text></view>
				</view>
	...
	export default {
		...
		methods: {
			...
			fUpload() {
				...
				// #ifdef H5
			/* 	x *= this.pixelRatio;
				y *= this.pixelRatio;  不需要进行转换*/
				expWidth = width;
				expHeight = height;
				// #endif
				 
				console.log("不知道什么原因暂时将y写死,观察几天");
				...
				uni.canvasToTempFilePath({
					...
					fileType: 'jpg',/* jpg 的清晰度比较低 png 好一点  暂时改一下jpg*/
					quality: 1,
					success: (r)=>{
					 //  console.info(r);
						r = r.tempFilePath;
